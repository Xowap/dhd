...
- **Raspberry Pi Pico 2** — This is tested on the Pico 2 (non-W) specifically. The Pico 1 has a different chip (RP2040 vs RP2350) so the firmware won't just flash as-is — you'd need to retarget it, which might range from trivial to annoying depending on what Embassy decides to do. The Pico 2 W should be closer to compatible since it's the same chip, but it's untested and who knows what the wireless hardware does to the USB stack or power delivery. Bottom line: if you use anything other than a plain Pico 2, you're in uncharted territory and some adaptation may be required.
