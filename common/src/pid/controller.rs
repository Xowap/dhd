...
impl<H: PidHardware, const N: usize> PidController<H, N> {
    ...
    /// Update the velocity estimate using an Exponential Moving Average (EMA) filter.
    ///
    /// This provides a smooth velocity signal for the derivative term, which is
    /// much less susceptible to ADC noise than differentiating the raw error.
    fn update_velocity_estimate(&mut self, measurement: f32) -> f32 {
        if !self.filt_init {
        ...
        self.filt_pos - self.filt_pos_prev
    }

    /// Check the hysteretic deadband to determine if the motor should be driven.
    ///
    /// This prevents the motor from "hunting" around the target due to ADC noise.
    /// - If currently driving, we only stop when within the inner band and velocity is low.
    /// - If currently idle, we only start driving when the error exceeds the outer band.
    fn should_drive(&mut self, error: f32, velocity: f32, range: f32) -> bool {
    ...
    /// Calculate the raw PID control effort.
    ///
    /// This includes proportional, integral (with anti-windup), derivative (on measurement),
    /// and feed-forward terms. The output represents the desired force *above* the stiction floor.
    fn calculate_pid_effort(
        &mut self,
        error: f32,
        velocity: f32,
        min_mag: f32,
        ...
        // Anti-windup: freeze integral if we are saturating in the direction of the error.
    ...
    /// Map the PID effort to a physical motor output, accounting for static friction.
    ///
    /// Since the motor will not move until the applied force exceeds stiction (`min_mag`),
    /// we shift the requested effort so that the smallest non-zero PID request translates
    /// to at least `min_mag` of physical force.
    fn apply_friction_mapping(
        &self,
        effort: f32,
        error_sign: f32,
        min_mag: f32,
        ...
        if effort.abs() < 1e-6 {
            ...
            // making progress, preventing a stall right before the target.
            error_sign * min_mag
        } else {
            let sign = if effort > 0.0 { 1.0 } else { -1.0 };
            let mag = (min_mag + effort.abs()).min(max_mag);
            sign * mag
        }
    }
    ...
    pub async fn step(&mut self) {
        ...
        let velocity = self.update_velocity_estimate(measurement);
    ...
    ///
    /// Stability is defined by the controller's internal hysteretic deadband and
    /// velocity threshold. Once reached, the hardware's `on_target_reached`
    /// method is called.
    pub async fn run_until_target(&mut self, target: f32) {
